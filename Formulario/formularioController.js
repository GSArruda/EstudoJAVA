angular.module("formulario").controller("formularioController", function ($scope){
			$scope.pessoas = [];

			$scope.adicionarPessoa = function (nome,sobrenome,email,senha,dia,mes,ano,sexo,local){
				$scope.pessoas.push({nome:nome,sobrenome:sobrenome,email:email,senha:senha,
					dia:dia,mes:mes,ano:ano,sexo:sexo,local:local});
				
					
			};

			$scope.anoAtual = (new Date()).getFullYear();

			$scope.verificarSenha = function (senha,senhac){
				console.log("Entrou na função");
				if (senha === senhac ){
					return true;
					console.log("retornou true");
				}else
					return false;
					console.log("retornou false");
			}

			$scope.listaMeses = [{nome: "Janeiro"},
				{nome:"Fevereiro"},
				{nome:"Março"},
				{nome:"Abril"},
				{nome:"Maio"},
				{nome:"Junho"},
				{nome:"Julho"},
				{nome:"Agosto"},
				{nome:"Setembro"},
				{nome:"Outubro"},
				{nome:"Novembro"},
				{nome:"Dezembro"}
			];

			$scope.listaSexos = [
				{nome:"Masculino"},
				{nome:"Feminino"}
			];

			$scope.listaLocais = [ 
			  {nome: 'Afghanistan'}, 
			  {nome: 'Åland Islands'}, 
			  {nome: 'Albania'}, 
			  {nome: 'Algeria'}, 
			  {nome: 'American Samoa'}, 
			  {nome: 'AndorrA'}, 
			  {nome: 'Angola'}, 
			  {nome: 'Anguilla'}, 
			  {nome: 'Antarctica'}, 
			  {nome: 'Antigua and Barbuda'}, 
			  {nome: 'Argentina'}, 
			  {nome: 'Armenia'}, 
			  {nome: 'Aruba'}, 
			  {nome: 'Australia'}, 
			  {nome: 'Austria'}, 
			  {nome: 'Azerbaijan'}, 
			  {nome: 'Bahamas'}, 
			  {nome: 'Bahrain'}, 
			  {nome: 'Bangladesh'}, 
			  {nome: 'Barbados'}, 
			  {nome: 'Belarus'}, 
			  {nome: 'Belgium'}, 
			  {nome: 'Belize'}, 
			  {nome: 'Benin'}, 
			  {nome: 'Bermuda'}, 
			  {nome: 'Bhutan'}, 
			  {nome: 'Bolivia'}, 
			  {nome: 'Bosnia and Herzegovina'}, 
			  {nome: 'Botswana'}, 
			  {nome: 'Bouvet Island'}, 
			  {nome: 'Brazil'}, 
			  {nome: 'British Indian Ocean Territory'}, 
			  {nome: 'Brunei Darussalam'}, 
			  {nome: 'Bulgaria'}, 
			  {nome: 'Burkina Faso'}, 
			  {nome: 'Burundi'}, 
			  {nome: 'Cambodia'}, 
			  {nome: 'Cameroon'}, 
			  {nome: 'Canada'}, 
			  {nome: 'Cape Verde'}, 
			  {nome: 'Cayman Islands'}, 
			  {nome: 'Central African Republic'}, 
			  {nome: 'Chad'}, 
			  {nome: 'Chile'}, 
			  {nome: 'China'}, 
			  {nome: 'Christmas Island'}, 
			  {nome: 'Cocos (Keeling) Islands'}, 
			  {nome: 'Colombia'}, 
			  {nome: 'Comoros'}, 
			  {nome: 'Congo'}, 
			  {nome: 'Congo, The Democratic Republic of the'}, 
			  {nome: 'Cook Islands'}, 
			  {nome: 'Costa Rica'}, 
			  {nome: 'Cote D\'Ivoire'}, 
			  {nome: 'Croatia'}, 
			  {nome: 'Cuba'}, 
			  {nome: 'Cyprus'}, 
			  {nome: 'Czech Republic'}, 
			  {nome: 'Denmark'}, 
			  {nome: 'Djibouti'}, 
			  {nome: 'Dominica'}, 
			  {nome: 'Dominican Republic'}, 
			  {nome: 'Ecuador'}, 
			  {nome: 'Egypt'}, 
			  {nome: 'El Salvador'}, 
			  {nome: 'Equatorial Guinea'}, 
			  {nome: 'Eritrea'}, 
			  {nome: 'Estonia'}, 
			  {nome: 'Ethiopia'}, 
			  {nome: 'Falkland Islands (Malvinas)'}, 
			  {nome: 'Faroe Islands'}, 
			  {nome: 'Fiji'}, 
			  {nome: 'Finland'}, 
			  {nome: 'France'}, 
			  {nome: 'French Guiana'}, 
			  {nome: 'French Polynesia'}, 
			  {nome: 'French Southern Territories'}, 
			  {nome: 'Gabon'}, 
			  {nome: 'Gambia'}, 
			  {nome: 'Georgia'}, 
			  {nome: 'Germany'}, 
			  {nome: 'Ghana'}, 
			  {nome: 'Gibraltar'}, 
			  {nome: 'Greece'}, 
			  {nome: 'Greenland'}, 
			  {nome: 'Grenada'}, 
			  {nome: 'Guadeloupe'}, 
			  {nome: 'Guam'}, 
			  {nome: 'Guatemala'}, 
			  {nome: 'Guernsey'}, 
			  {nome: 'Guinea'}, 
			  {nome: 'Guinea-Bissau'}, 
			  {nome: 'Guyana'}, 
			  {nome: 'Haiti'}, 
			  {nome: 'Heard Island and Mcdonald Islands'}, 
			  {nome: 'Holy See (Vatican City State)'}, 
			  {nome: 'Honduras'}, 
			  {nome: 'Hong Kong'}, 
			  {nome: 'Hungary'}, 
			  {nome: 'Iceland'}, 
			  {nome: 'India'}, 
			  {nome: 'Indonesia'}, 
			  {nome: 'Iran, Islamic Republic Of'}, 
			  {nome: 'Iraq'}, 
			  {nome: 'Ireland'}, 
			  {nome: 'Isle of Man'}, 
			  {nome: 'Israel'}, 
			  {nome: 'Italy'}, 
			  {nome: 'Jamaica'}, 
			  {nome: 'Japan'}, 
			  {nome: 'Jersey'}, 
			  {nome: 'Jordan'}, 
			  {nome: 'Kazakhstan'}, 
			  {nome: 'Kenya'}, 
			  {nome: 'Kiribati'}, 
			  {nome: 'Korea, Democratic People\'S Republic of'}, 
			  {nome: 'Korea, Republic of'}, 
			  {nome: 'Kuwait'}, 
			  {nome: 'Kyrgyzstan'}, 
			  {nome: 'Lao People\'S Democratic Republic'}, 
			  {nome: 'Latvia'}, 
			  {nome: 'Lebanon'}, 
			  {nome: 'Lesotho'}, 
			  {nome: 'Liberia'}, 
			  {nome: 'Libyan Arab Jamahiriya'}, 
			  {nome: 'Liechtenstein'}, 
			  {nome: 'Lithuania'}, 
			  {nome: 'Luxembourg'}, 
			  {nome: 'Macao'}, 
			  {nome: 'Macedonia, The Former Yugoslav Republic of'}, 
			  {nome: 'Madagascar'}, 
			  {nome: 'Malawi'}, 
			  {nome: 'Malaysia'}, 
			  {nome: 'Maldives'}, 
			  {nome: 'Mali'}, 
			  {nome: 'Malta'}, 
			  {nome: 'Marshall Islands'}, 
			  {nome: 'Martinique'}, 
			  {nome: 'Mauritania'}, 
			  {nome: 'Mauritius'}, 
			  {nome: 'Mayotte'}, 
			  {nome: 'Mexico'}, 
			  {nome: 'Micronesia, Federated States of'}, 
			  {nome: 'Moldova, Republic of'}, 
			  {nome: 'Monaco'}, 
			  {nome: 'Mongolia'}, 
			  {nome: 'Montserrat'}, 
			  {nome: 'Morocco'}, 
			  {nome: 'Mozambique'}, 
			  {nome: 'Myanmar'}, 
			  {nome: 'Namibia'}, 
			  {nome: 'Nauru'}, 
			  {nome: 'Nepal'}, 
			  {nome: 'Netherlands'}, 
			  {nome: 'Netherlands Antilles'}, 
			  {nome: 'New Caledonia'}, 
			  {nome: 'New Zealand'}, 
			  {nome: 'Nicaragua'}, 
			  {nome: 'Niger'}, 
			  {nome: 'Nigeria'}, 
			  {nome: 'Niue'}, 
			  {nome: 'Norfolk Island'}, 
			  {nome: 'Northern Mariana Islands'}, 
			  {nome: 'Norway'}, 
			  {nome: 'Oman'}, 
			  {nome: 'Pakistan'}, 
			  {nome: 'Palau'}, 
			  {nome: 'Palestinian Territory, Occupied'}, 
			  {nome: 'Panama'}, 
			  {nome: 'Papua New Guinea'}, 
			  {nome: 'Paraguay'}, 
			  {nome: 'Peru'}, 
			  {nome: 'Philippines'}, 
			  {nome: 'Pitcairn'}, 
			  {nome: 'Poland'}, 
			  {nome: 'Portugal'}, 
			  {nome: 'Puerto Rico'}, 
			  {nome: 'Qatar'}, 
			  {nome: 'Reunion'}, 
			  {nome: 'Romania'}, 
			  {nome: 'Russian Federation'}, 
			  {nome: 'RWANDA'}, 
			  {nome: 'Saint Helena'}, 
			  {nome: 'Saint Kitts and Nevis'}, 
			  {nome: 'Saint Lucia'}, 
			  {nome: 'Saint Pierre and Miquelon'}, 
			  {nome: 'Saint Vincent and the Grenadines'}, 
			  {nome: 'Samoa'}, 
			  {nome: 'San Marino'}, 
			  {nome: 'Sao Tome and Principe'}, 
			  {nome: 'Saudi Arabia'}, 
			  {nome: 'Senegal'}, 
			  {nome: 'Serbia and Montenegro'}, 
			  {nome: 'Seychelles'}, 
			  {nome: 'Sierra Leone'}, 
			  {nome: 'Singapore'}, 
			  {nome: 'Slovakia'}, 
			  {nome: 'Slovenia'}, 
			  {nome: 'Solomon Islands'}, 
			  {nome: 'Somalia'}, 
			  {nome: 'South Africa'}, 
			  {nome: 'South Georgia and the South Sandwich Islands'}, 
			  {nome: 'Spain'}, 
			  {nome: 'Sri Lanka'}, 
			  {nome: 'Sudan'}, 
			  {nome: 'Surinome'}, 
			  {nome: 'Svalbard and Jan Mayen'}, 
			  {nome: 'Swaziland'}, 
			  {nome: 'Sweden'}, 
			  {nome: 'Switzerland'}, 
			  {nome: 'Syrian Arab Republic'}, 
			  {nome: 'Taiwan, Province of China'}, 
			  {nome: 'Tajikistan'}, 
			  {nome: 'Tanzania, United Republic of'}, 
			  {nome: 'Thailand'}, 
			  {nome: 'Timor-Leste'}, 
			  {nome: 'Togo'}, 
			  {nome: 'Tokelau'}, 
			  {nome: 'Tonga'}, 
			  {nome: 'Trinidad and Tobago'}, 
			  {nome: 'Tunisia'}, 
			  {nome: 'Turkey'}, 
			  {nome: 'Turkmenistan'}, 
			  {nome: 'Turks and Caicos Islands'}, 
			  {nome: 'Tuvalu'}, 
			  {nome: 'Uganda'}, 
			  {nome: 'Ukraine'}, 
			  {nome: 'United Arab Emirates'}, 
			  {nome: 'United Kingdom'}, 
			  {nome: 'United States'}, 
			  {nome: 'United States Minor Outlying Islands'}, 
			  {nome: 'Uruguay'}, 
			  {nome: 'Uzbekistan'}, 
			  {nome: 'Vanuatu'}, 
			  {nome: 'Venezuela'}, 
			  {nome: 'Viet Nam'}, 
			  {nome: 'Virgin Islands, British'}, 
			  {nome: 'Virgin Islands, U.S.'}, 
			  {nome: 'Wallis and Futuna'}, 
			  {nome: 'Western Sahara'}, 
			  {nome: 'Yemen'}, 
			  {nome: 'Zambia'}, 
			  {nome: 'Zimbabwe'} 
			]


});